...
import { View, Text, FlatList, StyleSheet, Image, TouchableOpacity, Linking, ScrollView } from 'react-native';
...
const companyData = {
  ...
  description: 'TechCorp is a leading software company specializing in AI and blockchain.',
...
const CompanyProfileScreen = () => {
  ...
  const renderRecruiter = ({ item }) => (
    <View style={styles.recruiterCard} key={item.id}>
      {/* Add unique key prop */}
      <Image source={{ uri: 'https://www.example.com/recruiter-placeholder.png' }} style={styles.recruiterImage} />
      <CText fontWeight={600} sx={styles.recruiterName}>{String(item.name || 'No Name')}</CText>
      <CText sx={styles.recruiterPosition}>{String(item.position || 'No Position')}</CText>
      <TouchableOpacity onPress={()=>navigation.navigate("ChatReact")} style={styles.viewProfileButton}>
  ...
  const renderJobPost = ({ item }) => (
    <View style={styles.jobPostCard} key={item.id}>
      {/* Add unique key prop */}
      <CText fontWeight={600} sx={styles.jobPostTitle}>{String(item.title || 'No Title')}</CText>
      <CText sx={styles.jobPostDepartment}>{String(item.department || 'No Department')}</CText>
      <CText sx={styles.jobPostLocation}>{String(item.location || 'No Location')}</CText>
  ...
  const renderCompanyInfo = () => (
    <View style={styles.companyInfoContainer}>
      ...
      <CText sx={styles.companyName} fontWeight={600}>{companyData.name}</CText>
        ...
        <View>
          <ScrollView horizontal showsHorizontalScrollIndicator={false}>
            {item.content.map((recruiter) => (
              <View key={recruiter.id}>{renderRecruiter({ item: recruiter })}</View> // Ensure key is assigned here as well
            ))}
          </ScrollView>
