...
const validationSchema = Yup.object().shape({
  job_post_name: Yup.string()
    .required("Job name is required")
    .max(45, "Max length is 45 characters"),
  job_post_description: Yup.string().required("Description is required"),
  job_post_salary: Yup.string()
    .required("Salary is required")
    .max(45, "Max length is 45 characters"),
  job_post_location: Yup.string()
    .required("Location is required")
    .max(45, "Max length is 45 characters"),
  job_post_experience_level: Yup.string()
    .oneOf(["Experience", "Fresher", "Intern"], "Invalid Experience Level")
    .required("Experience level is required"),
  job_post_employment_type: Yup.string()
    .oneOf(["Full Time", "Part Time"], "Invalid Employment Type")
    .required("Employment type is required"),
  job_post_responsibility: Yup.array().of(
    Yup.string().required("Responsibility is required")
  ),
  job_post_requirement: Yup.array().of(
                ...
                <View style={styles.formContainer}>
                  ...
                  <View style={{ margin: 16 }}>
                    <Dropdown
                      ...
                      onSelect={(value) => {
                        setFieldValue("job_post_experience_level", value);
                      }}
                      mode={"outlined"}
                    />
                    <ErrorMessage
                  ...
                  <View style={{ marginHorizontal: 16 }}>
                    <Dropdown
                      ...
                      onSelect={(value) => {
                        setFieldValue("job_post_employment_type", value);
                      }}
                      mode={"outlined"}
                    />
                    <ErrorMessage
                  ...
                  <FieldArray
                    ...
                    render={(arrayHelpers) => (
                      <View style={{ marginHorizontal: 16 }}>
                        {values.job_post_responsibility.map(
                          (responsibility, index) => (
                            <View
                              key={index}
                              style={styles.responsibilityContainer}
                            >
                              <TextInput
                                label={`Role & Responsibility ${index + 1}`}
                                value={responsibility}
                                onChangeText={handleChange(
                                  `job_post_responsibility[${index}]`
                                )}
                                mode="outlined"
                              />
                              {index > 0 && (
                              ...
                              <ErrorMessage
                                name={`job_post_responsibility[${index}]`}
                                component={CText}
                                fontSize={12}
                                color={"red"}
                              />
                  ...
                  <FieldArray
                    ...
                    render={(arrayHelpers) => (
                      <View style={{ marginHorizontal: 16 }}>
                        {values.job_post_requirement.map(
                          (requirement, index) => (
                            <View
                              key={index}
                            ...
                            >
                              ...
                              <ErrorMessage
                                name={`job_post_requirement[${index}]`}
                                component={CText}
                                fontSize={12}
                                color={"red"}
                              />
